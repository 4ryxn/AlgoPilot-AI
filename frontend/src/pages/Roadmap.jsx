...
export default function Roadmap(){
  ...
  useEffect(()=>{api("/dashboard/planner").then(d=>setItems(d.items||[])).catch(e=>setError(e.message)).finally(()=>setLoading(false))},[]);
  return <main className="page"><p className="eyebrow">PLANNER</p><h1>Your Roadmap</h1><p className="muted">Daily practice, revision and company preparation.</p>
    ...
    <div className="roadmap-grid">
      <div className="card"><h2>Daily Planner</h2>{loading?<p className="muted">Loading planner...</p>:items.map((x,i)=><div className="plan-item" key={i}><div><strong>{x.title}</strong><p className="muted">{x.topic} · {x.difficulty}</p></div><span className="badge">Today</span></div>)}</div>
